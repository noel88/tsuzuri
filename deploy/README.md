...
초기화면과 결과 화면은 화살표로 옮기고 Enter로 고른다. 번호를 쳐서 가는
길도 그대로 있으니, 화살표가 안 먹혀도 앱을 쓸 수 있다. 화살표 조작이
터미널과 어긋나면 `TSUZURI_NO_ARROWS=1 tsuzuri` 로 끈다 — 그러면 예전처럼
번호를 치고 Enter를 누르는 방식만 남는다.

답을 쓰는 화면은 화살표 조작을 쓰지 않는다. 그 화면만은 터미널의 보통
입력 경로를 그대로 써야 한국어·일본어 입력기가 동작한다.
