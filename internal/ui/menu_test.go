...
func TestParseMenuKey(t *testing.T) {
	...
	// 번호를 다른 번호로 바꾸지 않는다. 「1」이 무엇을 뜻하는지는 그것을
	// 처리하는 쪽이 안다 — 여기서 바꾸면 화살표로 고른 길이 그 변환을
	// 거치지 않아 같은 항목이 다르게 동작한다.
	if k, ok := ParseMenuKey("1"); !ok || k != "1" {
		t.Errorf("ParseMenuKey(1) = %q,%v", k, ok)
	}
